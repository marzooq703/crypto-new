...
export default function CoinInput({
  ...
  const handleOnChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.value.match(decimalPattern)) {
      ...
      const param = { coin: defaultCoin, value: event.target.value };
  ...
  return (
    <div className="w-full">
      <div className="flex min-h-[70px] rounded-lg border border-gray-200 transition-colors duration-200 dark:border-gray-700">
        <div className="min-w-[80px] border-r border-gray-200 p-3 transition-colors duration-200 dark:group-hover:border-gray-600">
          <span className="mb-1.5 block text-xs uppercase text-gray-600 dark:text-gray-400">
            {label}
          </span>
          <span className="flex items-center font-medium outline-none dark:text-gray-100">
            <Tether className="w-6 h-6 mr-1" />
            USDT
          </span>
        </div>
